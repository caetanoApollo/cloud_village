document.addEventListener('DOMContentLoaded', function () {
    ...
    async function handleVehicleFormSubmit() {
        ...
        if (!plate || !model || !residentName || !spot) {
            showAlert('error', 'Preencha todos os campos obrigatórios');
            return;
        }
        ...
        try {
            const response = await fetch(`${API_BASE_URL}/veiculos`, {
                ...
                body: JSON.stringify({
                    placa: plate,
                    modelo: model.split(',')[0].trim(),
                    cor: model.split(',')[1]?.trim() || '',
    ...
    function updateResidentsTable(filteredResidents = residents) {
        ...
        filteredResidents.forEach(resident => {
            ...
            row.insertCell().textContent = resident.telefone;
    ...
    function updateVehiclesTable(filteredVehicles = vehicles) {
        ...
        filteredVehicles.forEach(vehicle => {
            ...
            row.insertCell().textContent = `${vehicle.modelo}${vehicle.cor ? ', ' + vehicle.cor : ''}`;
    ...
    window.editResident = async function(id) {
        ...
        document.getElementById('residentName').value = resident.nome;
        document.getElementById('residentBlock').value = `${resident.bloco}${resident.apartamento ? ', ' + resident.apartamento : ''}`;
        document.getElementById('residentContact').value = resident.telefone;
        document.getElementById('residentStatus').value = resident.status;

        // Remove o residente para edição
        try {
            ...
            showAlert('error', 'Falha ao preparar edição');
        }
    };
    ...
    window.editVehicle = async function(id) {
        ...
        document.getElementById('vehicleModel').value = `${vehicle.modelo}${vehicle.cor ? ', ' + vehicle.cor : ''}`;
        if (resident) {
            document.getElementById('vehicleResident').value = resident.nome;
        }
        document.getElementById('vehicleSpot').value = vehicle.box;

        // Remove o veículo para edição
        try {
            await fetch(`${API_BASE_URL}/veiculos/${id}`, {
                method: 'DELETE'
            });
